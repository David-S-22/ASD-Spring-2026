...
def _current_cycle_n(bill, today: date):
    """Return the cycle offset n (relative to bill.next_billing_date) of the latest
    occurrence <= today, or None if the bill has not started yet.
...
def current_cycle_due(bill, today: date):
    ...
    n = _current_cycle_n(bill, today)
    if n is None:
        return None
    return add_cadence(bill.next_billing_date, bill.cadence, n)
...
def derive_status(bill, payments, today: date):
    ...
    bill due exactly today is due; anything else is overdue. The next
    occurrence is always computed from the anchor at cycle index n + 1, not
    by re-adding a cadence step onto due_now, since chaining from an
    already-clamped date drifts for 31st-anchored monthly bills.
    """
    ...
    due_now = add_cadence(bill.next_billing_date, bill.cadence, n)
    ...
    if any(p.date >= due_now for p in bill_payments):
        ...
        next_occurrence = add_cadence(bill.next_billing_date, bill.cadence, n + 1)
